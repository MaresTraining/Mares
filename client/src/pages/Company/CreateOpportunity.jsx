...
const TrainingOpportunity = () => {
    ...
    const now = dayjs();
    const afterMonth = dayjs().add(1, 'month');
    const [startDate, setStartDate] = useState(now);
    const [endDate, setEndDate] = useState(afterMonth);

    const [formData, setFormData] = useState({
        generalSpecializationField: 'تقنية', // قيمة افتراضية للتخصص العام
        specificSpecializationField: 'برمجة',
        oppName: 'اسم الفرصة',
        ...
        trainingDuration: "",
        startDate: now ? now.format('YYYY-MM-DD') : '',
        endDate: afterMonth ? afterMonth.format('YYYY-MM-DD') : '',
        workingDays: "5",
        workingHours: "8",
        trainingPlan: "",
    ...
    const handleEndDate = (date) => {
        setEndDate(date);
        formData.endDate = date.format("YYYY-MM-DD")
        ...
        <Box>
            ...
            <Grid container direction="column" justifyContent="center" alignItems="center">
                ...
                <TextField
                    ...
                    onChange={(event) => setFormData({ ...formData, oppName: event.target.value })}
                    style={{ marginBottom: '20px', width: '50%' }}
                />
